...
__bash_prompt() {
    export NAME=$GITHUB_USER;
    local userpart='`export XIT=$? \
        && if [ "$NAME" = "$GITHUB_USER" ]; then \
            echo -n "\[\033[0;32m\]@${NAME:-} "; \
        elif [ ! -z "${NAME:-}" ]; then \
            echo -n "\[\033[0;32m\](${NAME:-}) "; \
        else \
            echo -n "\[\033[0;32m\]\u "; \
    ...
    local gitbranch='`\
        if [ "$NAME" = "$GITHUB_USER" ] && [ "$(git config --get devcontainers-theme.hide-status 2>/dev/null)" != 1 ] && \
    ...
    PS1="\n\[\033[0;33m\][\t] \[\033[0m\]${userpart} ${lightblue}\w ${gitbranch}${removecolor}\n\$ "
...
read -p "Masukkan nama Anda [$GITHUB_USER]: " NAME
NAME=${NAME:-$GITHUB_USER}

alias x='source /workspaces/mlbp-codespace/.devcontainer/instructions/x.sh'
